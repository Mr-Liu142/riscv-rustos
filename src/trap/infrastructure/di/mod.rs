...
/// Static storage for context manager
static mut CONTEXT_MANAGER: StandardContextManager = StandardContextManager::new();

/// Static storage for hardware control
static mut HARDWARE_CONTROL: RiscvHardwareControl = RiscvHardwareControl::new();
...
fn default_software_handler(ctx: &mut TrapContext) -> TrapHandlerResult {
    ...
    get_trap_system().get_hardware_control().clear_soft_interrupt();
    TrapHandlerResult::Handled
}
...
pub fn initialize_trap_system(mode: TrapMode) {
    ...
    // Create static references using raw pointers to static data
    let context_manager = unsafe { 
        StaticRef::from_static(&mut CONTEXT_MANAGER as *mut StandardContextManager) 
    };
    ...
    unsafe {
        TRAP_SYSTEM = Some(trap_system);
    }
    
    // Mark as initialized
    TRAP_SYSTEM_INITIALIZED.store(true, Ordering::SeqCst);
...
/// Get a reference to the trap system
///
/// # Panics
///
...
static mut CUSTOM_HANDLER_COUNT: usize = 0;

// Actual handler objects are stored in this array
static mut HANDLER_STORAGE: [Option<StandardTrapHandler>; MAX_CUSTOM_HANDLERS] = [None; MAX_CUSTOM_HANDLERS];
...
pub fn register_handler(
    ...
    // Create a static handler and register it with the trap system
    unsafe {
        ...
        HANDLER_STORAGE[CUSTOM_HANDLER_COUNT] = Some(handler);
        
        // Store reference in the references array
        if let Some(ref handler_instance) = HANDLER_STORAGE[CUSTOM_HANDLER_COUNT] {
            // Need to convert to 'static lifetime - this is safe because the storage is static
            let handler_ref: &'static StandardTrapHandler = core::mem::transmute(handler_instance);
            CUSTOM_HANDLERS[CUSTOM_HANDLER_COUNT] = Some(handler_ref);
            
            // Get a mutable reference to the trap system
            if let Some(trap_system) = TRAP_SYSTEM.as_mut() {
                let result = trap_system.register_handler(handler_ref);
                if result {
                    CUSTOM_HANDLER_COUNT += 1;
                }
                result
            } else {
                false
...
pub fn unregister_handler(trap_type: TrapType, description: &'static str) -> bool {
    // We need to get a mutable reference to the trap system
    unsafe {
        if let Some(trap_system) = TRAP_SYSTEM.as_mut() {
...
pub fn handler_count(trap_type: TrapType) -> usize {
    get_trap_system().handler_count_for_type(trap_type)
}

/// Print all registered handlers
pub fn print_handlers() {
    get_trap_system().print_handlers()
}

/// Enable interrupts
pub fn enable_interrupts() -> bool {
    get_trap_system().get_hardware_control().enable_interrupts()
}

/// Disable interrupts
pub fn disable_interrupts() -> bool {
    get_trap_system().get_hardware_control().disable_interrupts()
}

/// Restore interrupts
pub fn restore_interrupts(was_enabled: bool) {
    get_trap_system().get_hardware_control().restore_interrupts(was_enabled)
}

/// Enable a specific interrupt
pub fn enable_interrupt(interrupt: Interrupt) {
    get_trap_system().get_hardware_control().enable_interrupt(interrupt)
}

/// Disable a specific interrupt
pub fn disable_interrupt(interrupt: Interrupt) {
    get_trap_system().get_hardware_control().disable_interrupt(interrupt)
}

/// Check if an interrupt is enabled
pub fn is_interrupt_enabled(interrupt: Interrupt) -> bool {
    get_trap_system().get_hardware_control().is_interrupt_enabled(interrupt)
}

/// Check if an interrupt is pending
pub fn is_interrupt_pending(interrupt: Interrupt) -> bool {
    get_trap_system().get_hardware_control().is_interrupt_pending(interrupt)
}

/// Set a software interrupt
pub fn set_soft_interrupt() {
    get_trap_system().get_hardware_control().set_soft_interrupt()
}
...
pub fn create_task_context(entry: usize, user_stack: usize, kernel_stack: usize) -> TrapContext {
    get_trap_system().get_context_manager().create_task_context(
...
pub fn switch_task_context(current: &mut TaskContext, next: &TaskContext) {
    get_trap_system().get_context_manager_mut().switch_task_context(
        current, next
    )
}
...
pub fn is_in_interrupt_context() -> bool {
    get_trap_system().get_context_manager().is_in_interrupt_context()
}

/// Get the current interrupt nesting level
pub fn get_interrupt_nest_level() -> usize {
    get_trap_system().get_context_manager().get_nest_level()
}
...
pub fn internal_handle_trap(context: *mut TrapContext) {
    get_trap_system().handle_trap(context);
}
